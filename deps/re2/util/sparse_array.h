...
class SparseArray {
 public:
  ...
  iterator begin() { return dense_.get(); }
  iterator end() { return dense_.get() + size_; }

  const_iterator begin() const { return dense_.get(); }
  const_iterator end() const { return dense_.get() + size_; }
 ...
 public:
  ...
  iterator find(int i) {
    if (has_index(i)) return dense_.get() + sparse_to_dense_[i];
    return end();
  }
...
void SparseArray<Value>::erase(int i) {
  DebugCheckInvariants();
  if (has_index(i)) erase_existing(i);
  DebugCheckInvariants();
}
