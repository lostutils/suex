...
class Group {
 ...
 public:
  ...
  bool Contains(const User &user) const { return members_.find(user) != members_.end(); };
...
class Entity {
 public:
  ...
  explicit Entity(const User &user,
                  const User &as_user,
                  bool deny,
                  bool keepenv,
                  bool nopass,
                  bool persist,
                  const HashTable &env_to_add,
                  const Collection &env_to_remove,
